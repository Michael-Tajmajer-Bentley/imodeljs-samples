...
export class QueryAgent {
  ...
  private async _login(): Promise<AccessToken> {
    ...
    Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Getting JWT access token`);
    const jwt: AccessToken = await this._oidcClient!.getToken(actx);
    Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Got JWT access token`);
    return jwt;
  }
  ...
  public async listenForAndHandleChangesets(listenFor: number/*ms*/) {
    ...
    Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Setting up changeset and named version listeners...`);
    ...
    const deleteChangeSetListener = this._hubClient!.events.createListener(authCtx, async () => this._accessToken!, changeSetSubscription!.wsgId, this._iModelId!,
      async (receivedEvent: ChangeSetPostPushEvent) => {
        Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Received notification that change set "${receivedEvent.changeSetId}" was just posted on the Hub`);
        try {
          await this._extractChangeSummary(receivedEvent.changeSetId!);
        } catch (error) {
          Logger.logError(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Unable to extract changeset: ${receivedEvent.changeSetId}, failed with ${error}`);
        }
      });
    ...
    const deleteNamedVersionListener = this._hubClient!.events.createListener(authCtx, async () => this._accessToken!, namedVersionSubscription!.wsgId, this._iModelId!,
      async (receivedEvent: NamedVersionCreatedEvent) => {
        Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Received notification that named version "${receivedEvent.versionName}" was just created on the Hub`);
      });

    Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Listening to changesets for ${listenFor/1000.0} sec.`);
    ...
    Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Finished listening for changesets for ${listenFor/1000.0} sec.`);
  }
  ...
  private async _initialize(): Promise<void> {
    if (!this._isInitialized) {
      try {
        ...
        Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Attempting to find Ids for iModel and Project`);
        ...
        try {
          ...
          Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Project ${QueryAgentConfig.projectName} has id: ${projectId}`);
          ...
          Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Error: ${error}`);
          throw error;
        }
        if (projectId && iModelId) {
          this._projectId = projectId;
          this._iModelId = iModelId;
          Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Query Agent Initialized with event subscriptions for ${QueryAgentConfig.iModelName}`);
          this._isInitialized = true;
        }
      } catch (error) {
        const errorStr = `${new Date().toISOString()} Unable to verify IModel:'${QueryAgentConfig.iModelName}', for project '${QueryAgentConfig.projectName}' exists in the iModel Hub: ${error}`;
  ...
  private _writeChangeSummaryToDisk(content: any) {
    ...
    Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Wrote contents of change summary to ${filePath}`);
  }
