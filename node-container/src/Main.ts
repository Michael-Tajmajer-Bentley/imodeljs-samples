...
export const main = async (_process: NodeJS.Process, queryAgentWebServer?: QueryAgentWebServer, listenTime?: number,
    ...
    try {
        ...
        Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Query Agent Web Server exiting with error: ${error}`);
    ...
    queryAgentWebServer.close();
    if (success) {
        Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Query Agent Web Server finished executing successfully.`);
        processHandler.exitSuccessfully();
    }
    Logger.logTrace(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Query Agent Web Server finished executing unsuccessfully.`);
    processHandler.exitWithError();
};
