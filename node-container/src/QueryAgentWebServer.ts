...
export class QueryAgentWebServer {
  ...
  public constructor(webServer: express.Express = express(), agent: QueryAgent = new QueryAgent()) {
    ...
    this._server = webServer.listen(webServer.get("port"), () => Logger.logInfo(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} iModel Query Agent listening on http://localhost:` + webServer.get("port")))
      .on("error", (e: any) => this._handlePortError(QueryAgentConfig.port, e));
  }

  private _handlePortError = (port: number, error: Error) => {
    Logger.logError(QueryAgentConfig.loggingCategory, `${new Date().toISOString()} Unable to connect to port # ${port}. Make sure nothing else is listening on this port or try a different one... ${error}\n\n`);
    throw error;
  }
  ...
  public async run(listenTime?: number): Promise<boolean> {
    ...
    try {
      ...
      this.close();
      Logger.logError(QueryAgentConfig.loggingCategory, new Date().toISOString() + " " + error);
      return false;
    }
    return true;
  }
